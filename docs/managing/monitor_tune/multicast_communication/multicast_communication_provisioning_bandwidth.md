...
sidebar_label: Provisioning Bandwidth for Multicast
sidebar_position: 1
---
...
If some of your members have trouble keeping up with the incoming data, the other members in the group may be impacted. At best, slow receivers cause the producer to use buffering, adding latency for the slow receiver and perhaps for all of them. In the worst case, throughput for the group can stop entirely while the producerâ€™s CPU, memory and network bandwidth are dedicated to serving the slow receivers.

To address this issue, you can implement a bounded operation policy, which sets boundaries for the producerâ€™s operation. The appropriate rate limits are determined through tuning and testing to allow the fastest operation possible while minimizing data loss and latency in the group of consumers. This policy is suited to applications such as financial market data, where high throughput, reliable delivery and network stability are required. With the boundaries set correctly, your producerâ€™s traffic cannot cause a network outage.
...
<td><p>Tells consumers how low the producerâ€™s initial to remaining allowance ratio should be before sending a recharge.</p></td>
</tr>
<tr>
