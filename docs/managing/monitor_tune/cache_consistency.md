...
**Note:**
If you revoke a memberâ€™s disk store, do not restart that member with its disk storesâ€”in isolationâ€”at a later time.

 stores information about your persisted data and prevents you from starting a member with a revoked disk store in the running system. But  cannot stop you from starting a revoked member in isolation, and running with its revoked data. This is an unlikely situation, but it is possible to do:
...
6.  To get the system up and running, you start Member A, and use the command line tool to revoke Member Bâ€™s status as member with the most recent data. The system loads Member Aâ€™s data and you run forward with that.
...
Understanding the operation of  transactions can help you minimize situations where the cache could get out of sync.
...
An in-place change to directly alter a keyâ€™s value without doing a put can result in cache inconsistencies. With transactions, it creates additional difficulties because it breaks read committed isolation. If at all possible, use copy-on-read instead.
...
If a cache writer exists during a transaction, then each transaction write operation triggers a cache writerâ€™s related call. Regardless of the regionâ€™s scope, a transaction commit can invoke a cache writer only in the local cache and not in the remote caches.
