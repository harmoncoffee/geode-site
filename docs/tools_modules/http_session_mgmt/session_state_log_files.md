...
To add fine-level logging to the  Cache Client, add the 'log-level' property to the Listener element in the tc Server or Tomcat `server.xml` file. For example:
...
**Note:**
This will help debug  client issues, but it adds a lot of logging to the file.
