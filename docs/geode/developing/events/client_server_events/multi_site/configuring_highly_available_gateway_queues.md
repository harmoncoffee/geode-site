...
If you were to configure 10 dispatcher threads for the serial gateway sender, then the total maximum memory for the gateway sender queue would be 1000MB on each @@product_name@@ member that hosted the sender, because @@product_name@@ creates a separate copy of the queue per thread..
