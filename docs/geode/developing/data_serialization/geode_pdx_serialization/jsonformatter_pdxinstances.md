...
@@product_name@@ supports the use of JSON formatted documents natively. When you add a JSON document to a @@product_name@@ cache, you call the JSONFormatter APIs to transform them into the PDX format (as a `PdxInstance`), which enables @@product_name@@ to understand the JSON document at a field level.

In terms of querying and indexing, because the documents are stored internally as PDX, applications can index on any field contained inside the JSON document including any nested field (within JSON objects or JSON arrays.) Any queries run on these stored documents will return PdxInstances as results. To update a JSON document stored in @@product_name@@ , you can execute a function on the PdxInstance.
...
You need to call the following methods before putting any JSON document into the @@product_name@@ region:
...
After putting the JSON document into a region as a PdxInstance, you can execute standard @@product_name@@ queries and create indexes on the JSON document in the same manner you would query or index any other @@product_name@@ PdxInstance.

After executing a @@product_name@@ query or calling `region.get`, you can use the following methods to convert a PdxInstance back into the JSON format:
...
By default, @@product_name@@ serialization creates a unique pdx typeID for each unique JSON document, even if the
