...
sidebar_label: Deploying Configuration Files in JAR Files
sidebar_position: 4
---
...
1.  Jar the files.
2.  Set the @@product_name_long@@ system properties to point to the files as they reside in the jar file.
3.  Include the jar file in your `CLASSPATH.`
4.  Verify the jar file copies are the only ones visible to the application at runtime. @@product_name@@ searches the `CLASSPATH` after searching other locations, so the files cannot be available in the other search areas.
...
3.  Verify there is no file already  in the filesystem named `./myConfig/myCache.xml`, so @@product_name@@ will be forced to search the jar file to find it.
