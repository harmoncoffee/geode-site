...
You can select the most common region attributes settings from @@product_name@@’s predefined named region attributes in these classes:
...
-   **`HEAP_LRU`**. Causes least recently used data to be evicted from memory when the @@product_name@@ resource manager determines that the cache has reached configured storage limits.
