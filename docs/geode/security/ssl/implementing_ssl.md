...
distribution. You can use SSL alone or in conjunction with the other @@product_name@@ security options.

@@product_name@@ SSL connections use the Java Secure Sockets Extension (JSSE) package, so the properties
...
:    A boolean value that, when set to true, allows @@product_name@@ to use the default SSL context as returned by
...
The following table lists the properties you can use to configure SSL on your @@product_name@@ system.
...
| ssl&#8209;use&#8209;default&#8209;context | allows @@product_name@@ to use the default SSL context | boolean - if true, uses the default SSL context. Also sets ssl-endpoint-identification-enabled to true; defaults to false |
