...
1.  Start a `gfsh` prompt, using the version from your current @@product_name@@ installation, and connect to a currently running locator.
    For example:
...
1.  Shut down the entire cluster (by pressing Y at the prompt, this will lose no persisted data):
    ...
    Since @@product_name@@ is a Java process, to check before continuing that all @@product_name@@ members successfully stopped,
...
1. On each machine in the cluster, install any updated server code. Point all client applications to the new installation of @@product_name@@.
