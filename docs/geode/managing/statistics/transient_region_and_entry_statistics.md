...
For replicated, distributed, and local regions, @@product_name@@ provides a standard set of statistics for the region and its entries.

@@product_name@@ gathers these statistics when the `--enable-statistics` parameter of the `create region` command of `gfsh` is set to true or in cache.xml the region attribute `statistics-enabled` is set to true.
