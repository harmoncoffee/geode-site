...
## Cache Consistency and Concurrent Operations {#concept_49775DA33EFF4534950E1A60158F4B36__section_D1AB128682CA41D2BEFB429BED892ABE}
...
For example, modifications to region entries during an export can result in a snapshot that contains some but not all updates. If entries `{ A, B }` are updated to `{ A', B' }` during the export, the snapshot can contain `{ A, B' }` depending on the write order.

Also, modifications to region entries during an import can cause lost updates in the cache. If the region contains entries `{ A, B }` and the snapshot contains `{ A', B' }`, concurrent updates `{ A*, B* }` can result in the region containing `{ A*, B' }` after the import completes.
...
## Performance Considerations {#concept_49775DA33EFF4534950E1A60158F4B36__section_2C90FE1552D94AD08D1E5F3035153E0E}
