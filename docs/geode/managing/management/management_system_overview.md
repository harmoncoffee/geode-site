...
JConsole can connect to any @@product_name@@ member, but if it connects to a non-JMX-Manager member, JConsole only detects the local MBeans for the node, and not MBeans for the cluster.

When a @@product_name@@ locator or server becomes a JMX Manager for the cluster, it enables the RMI connector. JConsole can then connect only to that one JVM to view the MBeans for the entire cluster. It does not need to connect to all the other JVMs. @@product_name@@ manages the inter-JVM communication required to provide a federated view of all MBeans in the cluster.
...
You can use the @@product_name@@ management APIs to accomplish the following tasks:
...
-   Browse @@product_name@@ properties set for a particular member.
