...
Make sure that the machines that run @@product_name@@ members have enough CPU available to them. Do not run any other heavyweight processes on the same machine.

The machines that host @@product_name@@ application and cache server processes should have comparable computing power and memory capacity. Otherwise, members on the less powerful machines tend to have trouble keeping up with the rest of the group.
...
Upgrade the infrastructure to the level required for acceptable performance. Analyze the expected @@product_name@@ traffic in comparison to the network’s capacity. Build in extra capacity for growth and high-traffic spikes. Similarly, evaluate whether the machines that host @@product_name@@ application and cache server processes can handle the expected load.
