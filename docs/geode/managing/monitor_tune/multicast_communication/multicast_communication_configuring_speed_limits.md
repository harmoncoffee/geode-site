...
sidebar_label: Configuring Multicast Speed Limits
sidebar_position: 3
---
...
-   Increase the default datagram size for systems running Microsoft Windows from 1024 bytes to a value that matches your networkâ€™s maximum transmission unit (MTU), which is typically 1500 bytes. The higher setting should improve the systemâ€™s network performance.
...
-   Reduce multicast latency by disabling batching. By default, Geode uses batching for operations when the regionâ€™s scope is distributed-no-ack. Set the `disableBatching` property to true on the application or when starting a cache server process through the `gfsh` command line:
