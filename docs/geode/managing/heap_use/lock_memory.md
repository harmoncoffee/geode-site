...
2.  Using locked memory in this manner increases the time required to start Geode Geode . The additional time required to start  depends on the total amount of memory used, and can range from several seconds to 10 minutes or more. To improve startup time and reduce the potential of member timeouts, instruct the kernel to free operating system page caches just before starting a Geode member by issuing the following command:
