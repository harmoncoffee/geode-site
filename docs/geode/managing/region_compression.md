...
-   **Compress if you are storing large text values.** Compression is beneficial if you are storing large text values (such as JSON or XML) or blobs in @@product_name@@ that would benefit from compression.
...
In the Compressor element, specify the class-name for your compressor implementation. This example specifies the Snappy compressor, which is bundled with @@product_name@@ . You can also specify a custom compressor. See [Working with Compressors](#topic_hqf_syj_g4) for an example.
...
**Note:**
The Snappy codec included with @@product_name@@ cannot be used with Solaris deployments. Snappy is only supported on Linux, Windows, and macOS deployments of @@product_name@@.
