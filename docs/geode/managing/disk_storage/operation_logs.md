...
**Note:**
Log compaction can change the names of the disk store files. File number sequencing is usually altered, with some existing logs removed or replaced by newer logs with higher numbering. @@product_name@@ always starts a new log at a number higher than any existing number.
...
-   If auto-compaction is enabled, @@product_name@@ creates a new oplog in one of the directories, going over the limit, and logs a warning that reports:
    ...
    **Note:**
    When auto-compaction is enabled, `dir-size` does not limit how much disk space is used. @@product_name@@ will perform auto-compaction, which should free space, but the system may go over the configured disk limits.

-   If auto-compaction is disabled, @@product_name@@ does not create a new oplog, operations in the regions attached to the disk store block, and @@product_name@@ logs this error:
