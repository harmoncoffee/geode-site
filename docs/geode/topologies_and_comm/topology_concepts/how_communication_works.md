...
@@product_name@@ uses a combination of TCP and UDP unicast and multicast for communication between members. You can change the default behavior to optimize communication for your system.
...
In peer systems, for general messaging and region operations distribution, @@product_name@@ uses either TCP or UDP unicast. The default is TCP. You can use TCP or UDP unicast for all communications or you can use it as the default but then can target specific regions to use UDP multicast for operations distribution. The best combination for your installation depends in large part on your data use and event messaging.

## TCP {#how_communication_works__section_4402A20FEEC04055A0EEF6FEE82C116D}
...
**Note:**
Even when @@product_name@@ is configured to use UDP for messaging, @@product_name@@ uses a TCP connection when attempting to detect failed members. See [Failure Detection and Membership Views](../../managing/network_partitioning/failure_detection#concept_CFD13177F78C456095622151D6EE10EB) for more details. In addition, the TCP connection's ping is not used for keep alive purposes; it is only used to detect failed members. See [TCP/IP KeepAlive Configuration](../../managing/monitor_tune/socket_tcp_keepalive#topic_jvc_pw3_34) for TCP keep alive configuration.

## UDP Unicast and Multicast {#how_communication_works__section_E2D56EE03B54435BA9F04B8550F00534}
...
UDP is used in @@product_name@@ for both unicast and multicast messaging. @@product_name@@ implements retransmission protocols to ensure proper delivery of messages over UDP.

## UDP Unicast {#how_communication_works__section_F2393EE1280749F4B59E2558AA907526}
UDP unicast is the alternative to TCP for general messaging. UDP is more appropriate than TCP for unicast messaging when there are a large number of processes in the cluster, the network is not congested, cached objects are small, and applications can give the cache enough processing time to read from the network. If you disable TCP, @@product_name@@ uses UDP for unicast messaging.

For each member, @@product_name@@ selects a unique port for UDP unicast communication. You can restrict the range used for the selection by setting `membership-port-range` in the `gemfire.properties` file. Example:
...
**Note:**
In addition to UDP port configuration, the `membership-port-range` property defines the TCP port used for failure detection. See the [Reference](../../reference/book_intro#reference) for a description of the @@product_name@@ property.

## UDP Multicast {#how_communication_works__section_15F9EEDD65374F3E9D26C5A960D9D9D3}
...
Even if you use multicast for a region, @@product_name@@ will send unicast messages when appropriate. If data is partitioned, multicast is not a useful option. Even with multicast enabled, partitioned regions still use unicast for almost all purposes.
