...
## <a id="topic_7B158074B27A4FEF9D38E7C369905C72__section_17E7E4058D914334B9C5AC2E3DC1F7F2" class="no-quick-link"></a>Embedding a Gemcached server in a  Java Application

The `org.apache.geode.memcached` package contains a single class, `GemFireMemcachedServer` (see the  Javadocs.) Use this class to configure and embed a Gemcached server in a  cache server. For example, the following statement creates and starts an embedded Gemcached server on port number 5555 using the binary protocol:
...
| `memcached-protocol` | Memcached supports both ASCII and binary communication protocols (see the Memcached protocol documentation). By default, Gemcached uses the ASCII protocol.<br/><br/>Set one of the following values:<br/>• `ASCII` (default)<br/>• `BINARY` |
