...
-   **web.xml** needs a filter added as follows. If you have your own filters, the  Module filter **must** be the first one.
...
-   Add the following jar files from the AppServer.zip to the `WEB-INF/lib` directory of the war:
    ...
    -   slf4j-api jar
    -   slf4j-jdk14 jar
-   Add the following jar files from the `$GEODE/lib` directory to the `WEB-INF/lib` directory of the war, where `$GEODE` is set to the  product installation:
    -   antlr jar
    -   commons-io jar
    -   commons-lang jar
...
To run  in a peer-to-peer configuration, use the `modify_war` script with options
...
To run  in a client/server configuration, you make the application server operate as a  client. Use the `-t client-server` option to the `modify_war` script. This adds the following filter to application server's `web.xml` file:

To run  in a client/server configuration, you make the application server operate as a  client. Use the `modify_war` script with options
...
Once the application server is started, the  client will automatically launch within the application server process.

## <a id="weblogic_setting_up_the_module__section_3E186713737E4D5383E23B41CDFED59B" class="no-quick-link"></a>Verifying that  Started

You can verify that  has successfully started by inspecting the application server log file. For example:
...
Information is also logged within the  log file, which by default is named `gemfire_modules.<date>.log`.
