...
To run  in a client/server configuration, you make the application server operate as a Geode client. Use the `-t client-server` option to the `modify_war` script. This adds the following filter to application server's `web.xml` file:

To run Geode in a client/server configuration, you make the application server operate as a  client. Use the `modify_war` script with options
