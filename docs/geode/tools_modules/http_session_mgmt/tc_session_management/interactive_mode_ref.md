...
The above property determines whether a debug cache listener is added to the session region. When true, info-level messages are logged to the @@product_name@@ log when sessions are created, updated, invalidated, or expired.

``` pre
With the geode-p2p template:
  Please specify whether to maintain a local @@product_name@@ cache. Default 'false':
  
With the geode-cs template:
  Please specify whether to maintain a local @@product_name@@ cache. Default 'true':
```
...
``` pre
With the geode-p2p template:
  Please enter the id of the attributes of the @@product_name@@ region used to cache 
      sessions.
  Default 'REPLICATE':
...
The above property determines the @@product_name@@ region name.
...
``` pre
With the geode-p2p template:
  Please enter the name of the @@product_name@@ cache configuration file. 
...
``` pre
Please enter the list of locators used by @@product_name@@ members to discover each other. 
...
``` pre
  Please enter the name of the file used to log @@product_name@@ messages. 
  Default 'gemfire_modules.log':
```

The above property determines the file name for the @@product_name@@ log file.

``` pre
Applicable to the geode-p2p template ONLY:
  Please specify whether to rebalance the @@product_name@@ cache at startup.
  Default 'false':
```

This property allows you to rebalance a partitioned @@product_name@@ cache when a new @@product_name@@ peer starts up.

``` pre
  Please enter the name of the file used to store @@product_name@@ statistics. 
  Default 'gemfire_modules.gfs':
```

The above property determines the filename for the @@product_name@@ statistics file.

``` pre
  Please specify whether @@product_name@@ statistic sampling should be enabled. 
  Default 'false':
```
