...
-    peer-to-peer members are discovered using locators.
...
## <a id="weblogic_changing_gf_default_cfg__section_changing_sys_props" class="no-quick-link"></a>Changing  Distributed System Properties

To edit  system properties, you must add properties to  Session Filter definition in the application's web.xml file. As mentioned previously, this can be done by using the **-p** option to the `modify_war` script. All  system properties should be prefixed with the string **gemfire.property**. For example:
...
This example specifies that the file name for 's cache XML configuration is `cache-peer.xml`.

The list of configurable `server.xml` system properties include any of the properties that can be specified in 's `gemfire.properties` file. The following list contains some of the more common parameters that can be configured.
...
| locators (only for peer-to-peer config) | (required) list of locators (host\[port\]) used by  members; if a single locator listens on its default port, then set this value to `"localhost[10334]"` | Empty string                                                            |
...
In addition to the standard  system properties, the following cache-specific properties can also be configured.
...
Although these properties are not part of the standard  system properties, they apply to the entire JVM instance. For more information about managing the heap, refer to [Managing Heap and Off-heap Memory](../../managing/heap_use/heap_management.html#resource_manager).

**Note:**
It is important to note that the  cluster is a singleton within the entire application server JVM. As such it is important to ensure that different web applications, within the same container, set (or expect) the same cache configuration. When the application server starts, the first web application to start that uses  Session Caching will determine the overall configuration of the cluster, since it will trigger the creation of the cluster.
...
<dd>Whether to enable a debug listener in the session region; if this parameter is set to true, info-level messages are logged to the  log when sessions are created, updated, invalidated or expired.</dd>
