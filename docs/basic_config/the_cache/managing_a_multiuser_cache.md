...
In a single client, multiple authenticated users can all access the same `ClientCache` through instances of the `RegionService` interface. Because there are multiple users with varying authorization levels, access to cached data is done entirely through the servers, where each userâ€™s authorization can be managed.
...
1.  Create your cache and `RegionService` instances:
    1.  Configure your clientâ€™s server pool for multiple secure user authentication. Example:
    ...
    2.  After you create your `ClientCache`, from your `ClientCache` instance, for each user call the `createAuthenticatedView` method, providing the userâ€™s particular credentials. These are create method calls for two users:
    ...
    For each user, do all of your caching and region work through the assigned `RegionService` instance. Access to the server cache will be governed by the serverâ€™s configured authorization rules for each individual user.
